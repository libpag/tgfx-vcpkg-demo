...
function(build_tgfx_single_config SOURCE_PATH NODEJS OUTPUT_DIR IS_DEBUG WEB_ARCH)
    ...
    elseif(VCPKG_TARGET_ARCHITECTURE STREQUAL "wasm32")
        if(WEB_ARCH)
            set(ARCH "${WEB_ARCH}")
            message(STATUS "Using WebAssembly architecture: ${ARCH}")
        else()
            # wasm-st: single-thread tgfx
            # wasm-mt: multi-thread tgfx (default)
            set(ARCH "wasm-mt")
            message(STATUS "Using default WebAssembly architecture: ${ARCH}")
        endif()
    endif()
...
function(build_tgfx_with_vendor_tools SOURCE_PATH NODEJS WEB_ARCH)
    ...
    build_tgfx_single_config("${SOURCE_PATH}" "${NODEJS}" "${CURRENT_PACKAGES_DIR}/lib" FALSE "${WEB_ARCH}")
    if(NOT VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
        message(STATUS "Building TGFX debug version...")
        build_tgfx_single_config("${SOURCE_PATH}" "${NODEJS}" "${CURRENT_PACKAGES_DIR}/debug/lib" TRUE "${WEB_ARCH}")
    endif()
endfunction()
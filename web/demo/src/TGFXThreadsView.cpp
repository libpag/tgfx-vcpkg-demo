...
TGFXThreadsView::TGFXThreadsView(const std::string& canvasID) : TGFXBaseView(canvasID) {
    std::cout << "TGFXThreadsView constructor" << std::endl;
}

bool TGFXThreadsView::draw() {
    std::cout << "TGFXThreadsView::draw called" << std::endl;
    
    if (!emscripten_is_main_runtime_thread()) {
        std::cout << "Warning: draw called from worker thread" << std::endl;
        return false;
    }
